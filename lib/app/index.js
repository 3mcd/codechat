...
//defaultCode="<!doctype html><html><head><style>body { background: #2eb165; }.share-button { margin: auto; position: absolute;top: 0; left: 0; bottom: 0; right: 0;}</style><script src='http://cdnjs.cloudflare.com/ajax/libs/jquery/2.0.2/jquery.min.js'></script><script src='https://dl.dropboxusercontent.com/u/21098170/Scripts/share.min.js'></script></head><body><div class='share-button'></div><script>$(function(){$('.share-button').share({title: 'Share CodeChat',image: 'http://carrot.is/img/fb-share.jpg',app_id: '',background: 'rgba(255,255,255,.5)',color: '#3B2B45'});});</script></body></html>"
...
// Can do stuff when particular views load.
...
app.get('/codechat/:room?', function(page, model, params, next) {
	...
	var messagesQuery, code, room = params.room;
	...
	code.subscribe(function(err) {
		...
		if (!model.get('code.' + room)) {
			model.set('code.' + room + '.html', 'Some Default Code');
		}
	...
	// Subscribe to the messagesQuery
	return messagesQuery.subscribe(function(err) {
		var usersQuery, trueUserQuery;
		if (err) { return next(err); } 

		trueUserQuery = model.query('users', { room: room });

		trueUserQuery.subscribe(function(err){
			if(err) { return next(err); }
			trueUserQuery.ref('_page.trueUsers');
		});

		// the messages model, the timeSort function declared in app.on('model') by model.fn('timeSort'),
		...
		return usersQuery.subscribe(function(err) {
			var user, userCount;
			if (err) { return next(err); } 
			...
			return userCount.fetch(function(err) {
				...
				return userCount.increment(function(err) {
					...
					user.set({
						...
						userColor: (userCount.get() % numUserColors),
						room: room
					});
					return page.render('codechat');
				});
...
app.fn('serverMessage', function(comment, room) {
	comment ? comment : 'The server emitted a message.';
	this.model.add('messages', {
		room: room,
		userId: 'ccserver',
		comment: comment,
